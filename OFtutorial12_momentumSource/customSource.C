...
namespace fv
{
    defineTypeNameAndDebug(customSource, 0);
...
void Foam::fv::customSource::checkData() const
{
...
Foam::fv::customSource::customSource
(
    ...
    V_(0.0)
{
    ...
    dimensionedVector x0 ("x0", dimLength,diskCenter_);
    ...
    for (label cellI = 0; cellI < mesh_.C().size(); cellI++)
    {
        ...
        if ((mag(dNormal) < t_/2.) && (dRadial < diameter_/2.))
            cells_.append(cellI);
    }
...
void Foam::fv::customSource::addSup
(
    fvMatrix<vector>& eqn,
    const label fieldi
)
...
bool Foam::fv::customSource::read(const dictionary& dict)
{
    if (option::read(dict))
    {
        coeffs_.readIfPresent("diskDir", diskDir_);
        coeffs_.readIfPresent("diskCentre",diskCenter_);
        coeffs_.readIfPresent("D", diameter_);
